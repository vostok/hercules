 ...
 * @author Petr Demenev
 */
public class SentryConnectorHolder {
     ...
     */
    private volatile ConcurrentMap<String, ConcurrentMap<String, SentryConnector>> connectors = new ConcurrentHashMap<>();
    private final SentryApiClient sentryApiClient;
    private final SentryClientFactory sentryClientFactory;

    public SentryConnectorHolder(SentryApiClient sentryApiClient, Properties senderProperties) {
     ...
     */
    public Result<SentryConnector, ErrorInfo> getOrCreateConnector(String organization, String project) {
        ConcurrentMap<String, SentryConnector> projectMap = connectors.get(organization);
        if (projectMap == null) {
            ...
            Result<ConcurrentMap<String, SentryConnector>, ErrorInfo> orgExists = createOrganizationIfNotExists(organization);
            if (!orgExists.isOk()) {
                return Result.error(orgExists.getError());
            }
            projectMap = orgExists.get();
        }
        SentryConnector connector = projectMap.get(project);
        if (!projectMap.containsKey(project)) {
            Result<Void, ErrorInfo> nameValidationResult = validateSlug(project);
            if (!nameValidationResult.isOk()) {
                return Result.error(nameValidationResult.getError());
            }
            Result<SentryConnector, ErrorInfo> projectExists = createProjectIfNotExists(organization, project);
     ...
     */
    private Result<ConcurrentMap<String, SentryConnector>, ErrorInfo> createOrganizationIfNotExists(String organization) {
     ...
     */
    private Result<SentryConnector, ErrorInfo> createProjectIfNotExists(String organization, String project) {
        ...
        Result<SentryConnector, ErrorInfo> clientResult = createClient(organization, project);
     ...
     */
    private Result<SentryConnector, ErrorInfo> createClient(String organization, String project) {
        ...
        if (dsnOptional.isPresent()) {
            ...
            return Result.ok(SentryConnector.of(sentryClient));
        } else {
            return Result.ok(SentryConnector.empty());
        }
    }
    ...
    public void removeClientFromCache(SentryDestination destination) {
        ...
        connectors.get(organization).put(project, SentryConnector.empty());
    ...
    public void update() {
        try {
            ...
            ConcurrentMap<String, ConcurrentMap<String, SentryConnector>> organizationMap = new ConcurrentHashMap<>();
            for (OrganizationInfo organizationInfo : organizations.get()) {
                ...
                ConcurrentMap<String, SentryConnector> projectMap = new ConcurrentHashMap<>();
                ...
                for (ProjectInfo projectInfo : projects.get()) {
                    ...
                    Result<SentryConnector, ErrorInfo> clientResult = createClient(organization, project);
                    if (clientResult.isOk()) {
                        SentryConnector client = clientResult.get();
